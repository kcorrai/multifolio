...
- `app/dashboard/{page,profile,jobs}/page.tsx` — her sekme ayrı route (sunucu): yalnızca kendi veri dilimini çeker → ilgili `*-tab` client bileşenine iletir. `page.tsx` = Genel Bakış (`/dashboard`); eski Analytics içeriği (tür bazlı kullanım + 30 günlük grafik + başvuru performansı) buraya birleştirildi. **Portfolio ve Analytics sekmeleri kaldırıldı** (portfolio yalnız UI — arka uç duruyor). `profile/page.tsx` çekirdek profil + `platform_profiles`'ı (çekilmiş public profiller) çeker → `ProfileTab`: kimlik hero'su (avatar+headline+bağlı platform rozetleri+tamamlanma halkası) + form + AI öneri paneli + bağlı public profiller listesi.
