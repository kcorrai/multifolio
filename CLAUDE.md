...
- `lib/feed/relevance.ts` — SAF profil×ilan alaka motoru (AI/kredi YOK): `jobRelevance` (skill kesişimi + başlık bonus → 0-100) + `orderDefaultFeed` (feedsiz görünümü relevance DESC sıralar + düşük alakalıyı gizler, ≥3 skill sinyali varsa). Feed/search/starred route + SSR loader `PoolJob.relevance` doldurur; pool-job-panel rozet + düşük-alaka skorlama uyarısı (kredi israfını önler). Dashboard P0-1/P0-2.
