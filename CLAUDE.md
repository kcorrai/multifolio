...
- `lib/ai/` — uyarlama motoru (sunucu-only): `openai-client.ts` (OpenAI gpt-4o-mini istemcisi),
  ...
  `match.ts` (`matchJobToProfile` + ilandan `requirements` çıkarımı; 4 boyutlu rubrik üretir + ilan bağlamı `MatchJobContext` alır), `portfolio.ts` (`generatePortfolio` — dashboard UI GERİ GETİRİLDİ `components/dashboard/portfolio-tab.tsx` + `/dashboard/portfolio`; **OpenAI structured-output ayrı `portfolioGenSchema` kullanır — depolama şeması `.optional()` url'i OpenAI'ı 400'ler; sonuç `mapToContent` ile PortfolioContent'e kırpılır**), `rubric.ts` (SAF, server-only değil: `RUBRIC_WEIGHTS` %40/30/20/10 + `computeRubricScore` — toplam skor rubrikten deterministik + `rubricVerdict` go/maybe/skip; UI da import eder), `proposal.ts` (`generateProposal` — teklif metni + ilan gereksinimlerine karşı kapsama),
