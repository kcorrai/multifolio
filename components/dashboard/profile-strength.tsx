  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-bold tabular-nums ${BAND_STYLE[r.band]}`}>
      ...
      <ul className="space-y-1">
        {sorted.map((c) => (
          <li key={c.id} className={`flex items-start gap-1.5 text-xs ${c.passed ? "text-muted-foreground/60" : "text-foreground"}`}>
            {c.passed ? (
