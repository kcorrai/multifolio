...
function PlatformBadge({ source }: { source: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[11px] font-semibold capitalize ${
    ...
    >
      <div className="flex items-start gap-2.5">
          ...
          {job.skills.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {job.skills.slice(0, 4).map((s) => (
                <span key={s} className="rounded-md bg-muted px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground">{s}</span>
              ))}
            </div>
          )}
        </div>
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          ...
          {job.score !== null ? (
            <span className={`text-[11px] font-bold rounded-md px-1.5 py-0.5 tabular-nums ${scoreColor(job.score)}`}>{job.score}</span>
          ) : job.relevance !== null ? (
            ...
            <span
              ...
              className={`text-[11px] font-semibold rounded-md px-1.5 py-0.5 tabular-nums ${
