      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-2">
          ...
          <Textarea
            ...
            className="resize-none text-sm leading-relaxed"
          />
          <p className="text-xs text-muted-foreground/70">{t("jobHint")}</p>
        </div>
        <p className="flex items-start gap-2 text-xs text-emerald-600 dark:text-emerald-400 leading-relaxed">
        ...
        <div className="rounded-2xl border border-[#00F0FF]/20 bg-[#00F0FF]/5 p-5 space-y-4">
          ...
          {!locked && has && report.missingKeywords.length > 0 && (
            <div className="pt-2 border-t border-[#00F0FF]/15 space-y-1.5">
              <p className="text-xs font-semibold text-amber-600 dark:text-amber-400">{t("missingLabel")}</p>
              <div className="flex flex-wrap gap-1.5">
                {report.missingKeywords.map((k) => (
                  <span key={k} className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium text-amber-700 dark:text-amber-300">
                    {k}
                  </span>
                ))}
              </div>
