...
// edilir; karta tıklanınca Upwork tarzı detay modalı açılır (rol + açıklama + beceriler +
// görsel galerisi). Görsele tıklanınca ImageLightbox ile tam ekran büyür/gezinir.
// Server sayfası (/p/[slug]) yalnız proje gruplarını + yedek alt metni geçer; tema
// --pf-* CSS değişkenlerinden gelir (server'daki portfolioTheme).
  ...
  return (
    <>
      ...
      {open !== null && projects[open] && (
        <ProjectModal project={projects[open]} fallbackAlt={fallbackAlt} onClose={() => setOpen(null)} />
      )}
    </>
  );
}

// Upwork tarzı proje detay modalı: sol sütun rol/açıklama/beceriler, sağ sütun görseller.
function ProjectModal({
  project, fallbackAlt, onClose,
}: {
