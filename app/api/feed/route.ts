...
export const GET = withErrorHandler(async (req) => {
  ...
  const [feedsRes, poolRes, starRes, scoreRes] = await Promise.all([
    supabase.from("job_feeds").select("id, name, keywords, min_budget, platform, exclude_countries, min_hourly_rate, min_fixed_price, min_client_spent, min_score, notify, created_at").eq("user_id", user.id).order("created_at", { ascending: false }),
