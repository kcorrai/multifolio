    ...
    <div className="sp-page">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-8 pt-20 pb-4 text-center space-y-4">
          <p className="anim-fade-up anim-d0 text-xs font-bold uppercase tracking-[0.2em] text-[#FA531C]">{t("heroEyebrow")}</p>
          <h1 className="anim-fade-up anim-d1 text-4xl lg:text-5xl font-extrabold leading-[1.1] tracking-tight">{t("heroTitle")}</h1>
          <p className="anim-fade-up anim-d2 text-lg text-slate-500 dark:text-[#6B4A3C] leading-relaxed max-w-xl mx-auto font-medium">
            {t("heroSubtitle")}
          </p>
        </div>
      </section>
      ...
      <section className="border-y border-slate-200 dark:border-white/5 bg-slate-100/60 dark:bg-[#FFFAF3]/40 py-24">
        <div className="mx-auto max-w-3xl px-8">
          <ScrollReveal>
            <div className="text-center space-y-3 mb-12">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-pink-400">{t("costsEyebrow")}</p>
              <h2 className="text-4xl font-extrabold tracking-tight">{t("costsTitle")}</h2>
              <p className="text-slate-500 dark:text-[#6B4A3C] text-lg max-w-xl mx-auto font-medium">{t("costsSubtitle")}</p>
            </div>
          </ScrollReveal>

          <ScrollReveal delay={80}>
            <div className="rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-[#FFFAF3] overflow-hidden">
              ...
              {COST_ROWS.map(({ kind, icon: Icon }, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-xl bg-[#FA531C]/10 border border-[#FA531C]/20 flex items-center justify-center shrink-0">
                      <Icon className="h-4 w-4 text-[#FA531C]" />
                    </div>
          ...
          <ScrollReveal delay={160}>
            <p className="mt-5 text-center text-sm text-slate-500 dark:text-[#6B4A3C] font-medium">{t("actionsNote")}</p>
          </ScrollReveal>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-8 py-24 text-center">
        <ScrollReveal scale>
          <div className="relative rounded-[var(--radius-sp-xl)] bg-[var(--surface-card-peach)] px-8 py-16 overflow-hidden">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute left-1/3 top-1/2 -translate-y-1/2 h-[300px] w-[300px] rounded-full bg-[#FA531C]/6 blur-[80px]" />
              <div className="absolute right-1/3 top-1/2 -translate-y-1/2 h-[300px] w-[300px] rounded-full bg-pink-500/8 blur-[80px]" />
            </div>
            <div className="relative space-y-5">
              <h2 className="text-4xl font-extrabold tracking-tight">{t("ctaTitle")}</h2>
              <p className="text-slate-500 dark:text-[#6B4A3C] text-lg font-medium max-w-md mx-auto">{t("ctaSubtitle")}</p>
              <Link
                href={isLoggedIn ? "/dashboard" : "/signup"}
                className="anim-neon-pulse inline-flex items-center h-12 px-8 rounded-xl text-base font-bold bg-[#FA531C] text-[#3A241B] hover:bg-[#E23C05] transition-colors mt-2"
              >
