...
import { Layers, Gauge, Sparkles, ArrowRight, Check } from "lucide-react";
...
export default async function FreelancePseoPage({ params }: PageProps) {
  ...
  const features = [
    { icon: Layers, title: t("featureAdaptTitle", vars), body: t("featureAdaptBody", vars), accent: "#FA531C" },
    { icon: Gauge, title: t("featureScoreTitle", vars), body: t("featureScoreBody", vars), accent: "#a78bfa" },
    { icon: Sparkles, title: t("featureProposeTitle", vars), body: t("featureProposeBody", vars), accent: "#FA531C" },
  ];
  const faqs = [
    ...
    <div className="sp-page">
      ...
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute right-1/4 top-10 h-[350px] w-[350px] rounded-full bg-pink-500/8 blur-[100px]" />
        </div>
        <div className="relative mx-auto max-w-3xl px-8 pt-20 pb-10 text-center space-y-5">
          <div className="inline-flex items-center gap-2 rounded-full border border-[#FA531C]/25 bg-[#FA531C]/8 px-3.5 py-1.5">
          ...
          <p className="text-lg text-slate-500 dark:text-[#6B4A3C] leading-relaxed max-w-xl mx-auto font-medium">{t("subtitle", vars)}</p>
          <div className="flex flex-wrap items-center justify-center gap-3 pt-1">
            <Link href={`/signup?ref=freelance-${platform}`} className="anim-neon-pulse inline-flex h-12 items-center rounded-xl bg-[#FA531C] px-7 text-base font-bold text-[#3A241B] hover:bg-[#E23C05] transition-colors">
              {t("ctaPrimary")} <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
            <Link href="/analyze" className="inline-flex h-12 items-center rounded-xl border border-pink-500/40 px-7 text-base font-semibold text-pink-500 dark:text-pink-400 hover:bg-pink-500/10 transition-colors">
              {t("ctaSecondary")}
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-3xl px-8 py-10">
        <div className="rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-[#FFFAF3] p-6 sm:p-8">
          <h2 className="text-2xl font-bold tracking-tight">{t("problemTitle", vars)}</h2>
          <p className="mt-3 text-slate-500 dark:text-[#6B4A3C] leading-relaxed font-medium">{t("problemBody", vars)}</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-8 py-8">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {features.map((f) => (
            <div key={f.title} className="rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-[#FFFAF3] p-6 space-y-3">
              ...
              <p className="text-sm text-slate-500 dark:text-[#6B4A3C] leading-relaxed font-medium">{f.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-8 py-10">
        ...
        <p className="mt-1.5 text-sm text-slate-500 dark:text-[#6B4A3C] font-medium">{t("skillsBody", vars)}</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {l.role.skills.map((s) => (
            <span key={s} className="inline-flex items-center gap-1.5 rounded-full border border-[#FA531C]/25 bg-[#FA531C]/8 px-3 py-1 text-sm font-semibold text-slate-700 dark:text-[#FA531C]">
              <Check className="h-3.5 w-3.5 text-[#FA531C]" />{s}
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-8 py-10">
        ...
        <div className="space-y-3">
          {faqs.map((f) => (
            <details key={f.q} className="group rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-[#FFFAF3] px-5 py-4">
              <summary className="flex items-center justify-between cursor-pointer list-none font-semibold">
                {f.q}
                <span className="ml-4 shrink-0 text-[#FA531C] transition-transform group-open:rotate-45 text-xl leading-none">+</span>
              </summary>
              <p className="mt-3 text-sm text-slate-500 dark:text-[#6B4A3C] leading-relaxed font-medium">{f.a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-8 pb-24 text-center">
        <div className="relative overflow-hidden rounded-3xl border border-[#FA531C]/15 bg-white dark:bg-[#FFFAF3] px-8 py-14">
          ...
          <div className="relative space-y-4">
            ...
            <p className="mx-auto max-w-md text-slate-500 dark:text-[#6B4A3C] font-medium">{t("finalBody")}</p>
            <Link href={`/signup?ref=freelance-${platform}`} className="anim-neon-pulse inline-flex h-12 items-center rounded-xl bg-[#FA531C] px-8 text-base font-bold text-[#3A241B] hover:bg-[#E23C05] transition-colors mt-1">
