...
export default async function JobsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  const [profileRes, jobsRes, feedsRes, poolRes, starRes, scoreRes] = await Promise.all([
    ...
    supabase.from("job_listings").select("id, title, company, platform, status, match_score, match_result, created_at, reminder_date, deadline_date").eq("user_id", user.id).order("created_at", { ascending: false }),
